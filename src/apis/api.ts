...
const useApi = <T, P>({
  ...
  const concat = (init: ProcessParams) => {
    return (keyToConcat: keyof P | null = null, before: ProcessParams = init) => {
        ...
        new Promise((resolve, reject) => {
          ;(async () => {
            ...
            try {
              ...
              setData((value) => {
                if (concat && value) {
                  if (value[concat as keyof P] && Array.isArray(value[concat as keyof P])) {
                    return {
                      ...value,
                      [concat as keyof P]: (value[concat as keyof P] as Array<unknown>).concat(
                        data![concat as keyof P]
                      )
                    }
                  }
